...
from mangacollec_api.auth.auth_interface import IAuth
    ...
    def authenticate_with_password(
        ...
        try:
            response = requests.post(
                self.TOKEN_URL, headers=self.DEFAULT_HEADERS, data=payload
            )
...
if __name__ == "__main__":
    ...
    if not client_id or not client_secret:
        print(
            "Erreur: MANGACOLLEC_CLIENT_ID et MANGACOLLEC_CLIENT_SECRET doivent être définis"
        )
        exit(1)
